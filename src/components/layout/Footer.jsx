...
export default function Footer({ config = {}, images = {}, dict = {} }) {
    ...
    return (
        <footer role="contentinfo" className="font-sans overflow-hidden flex flex-col">
            {/* Main Footer Content - Green Background */}
            <div className="bg-[var(--primary-green)] text-white py-8 sm:py-12">
                <motion.h1
                ...
                >
                    <div className="flex flex-wrap justify-center gap-6 sm:gap-12 text-base sm:text-lg font-normal">
                        {(mergedConfig.navLinks && mergedConfig.navLinks.length
                            ...
                            let label = link.name
                            if (dict?.nav) {
                                if (link.path === '/') label = dict.nav.home
                                else if (link.path.includes('about')) label = dict.nav.about
                                else if (link.path.includes('services')) label = dict.nav.services
                                else if (link.path.includes('careers')) label = dict.nav.careers
                                else if (link.path.includes('contact')) label = dict.nav.contact
                            }
                            return (
            ...
            <div className="bg-white py-4 border-t border-gray-100">
                <motion.div
                    className="text-center text-sm sm:text-base md:text-md lg:text-lg font-normal text-gray-600 hover:text-gray-800 transition-colors px-4"
                    initial="hidden"
                    whileInView="visible"
